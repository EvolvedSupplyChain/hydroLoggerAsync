...
def sub_cb(topic, msg):
  ...
  if topic.decode() == ccTopic:
    ...
    try:
        ...
        client.publish(feedbackTopic, json.dumps({"subject":subject,"message":decodedMsg}).encode())
    except Exception as error:
        print(error)
    ...
    elif subject == "arbExec":
        try:
            print(decodedMsg.get("command"))
            exec(decodedMsg.get("command"))
        except Exception as error:
            print(error)
...
#theCallbacks = callbackFuncs(telemTopic,logTopic,statusTopic,feedbackTopic,ccTopic,config,client)
